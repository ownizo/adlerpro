...
function PartnerRiskPage() {
  ...
  const analyzeRisk = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/analyze-partner', {
        ...
        body: JSON.stringify({ nif: nif.trim(), cprc: cprc.trim(), lang }),
      })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || t('partnerRisk.errors.analysisFailed'))
      setReport(data.report)
      setCompanyName(data.companyName)
      setCompanyProfile(data.companyProfile)
    } catch (err: any) {
      setError(err.message || t('partnerRisk.errors.generic'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePrint = () => {
    ...
    if (!w) return
    w.document.write(
      '<!DOCTYPE html><html><head><title>' + t('partnerRisk.reportTitle') + ' — ' + (companyName || '') + '</title>' +
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', padding: '1rem 1.25rem', background: '#f8f8f8', border: '1px solid #eeeeee', borderRadius: '4px', marginBottom: '1.5rem' }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.8rem', color: '#666', margin: 0, lineHeight: 1.5 }}
              dangerouslySetInnerHTML={{ __html: t('partnerRisk.howItWorksDesc') }}
            />
          </div>
        </div>
          ...
          <form onSubmit={analyzeRisk}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontFamily: "'Montserrat', sans-serif", fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#111', marginBottom: '0.5rem' }}>
                  {t('partnerRisk.nif')} <span style={{ color: '#C8961A' }}>*</span>
                </label>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    placeholder={t('partnerRisk.nifPlaceholder')}
                    value={nif}
                    onChange={(e) => setNif(e.target.value.replace(/\D/g, '').slice(0, 9))}
                    maxLength={9}
                  />
                </div>
                <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.7rem', color: '#888', margin: '0.25rem 0 0' }}>{t('partnerRisk.nifRequired')}</p>
              </div>
              <div>
                <label style={{ display: 'block', fontFamily: "'Montserrat', sans-serif", fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#111', marginBottom: '0.5rem' }}>
                  {t('partnerRisk.cprc')} <span style={{ color: '#888', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>{t('partnerRisk.cprcOptional')}</span>
                </label>
                <div style={{ position: 'relative' }}>
                  <div style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#999' }}>
                  ...
                  <input
                    type="text"
                    style={{ width: '100%', padding: '0.625rem 0.75rem 0.625rem 2.25rem', border: '1px solid #ddd', borderRadius: '4px', fontFamily: "'Montserrat', sans-serif", fontSize: '0.9rem', color: '#111', outline: 'none', boxSizing: 'border-box' }}
                    placeholder={t('partnerRisk.cprcPlaceholder')}
                    value={cprc}
                    onChange={(e) => setCprc(e.target.value)}
                  />
                </div>
                <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.7rem', color: '#888', margin: '0.25rem 0 0' }}>{t('partnerRisk.cprcHint')}</p>
              </div>
            </div>
            <button
              type="submit"
            ...
            >
              {loading ? (
                ...
                <><Search style={{ width: '16px', height: '16px' }} />{t('partnerRisk.analyzeBtn')}</>
              )}
            </button>
            {error && (
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <h3 style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.9rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#111', margin: 0 }}>{t('partnerRisk.reportTitle')}</h3>
              </div>
        ...
        {!report && !loading && (
          <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
            ...
            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.95rem', fontWeight: 600, color: '#555', marginBottom: '0.5rem' }}>{t('partnerRisk.emptyState')}</p>
            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.85rem', color: '#888', lineHeight: 1.5 }}>{t('partnerRisk.emptyStateHint')}</p>
          </div>
        )}
      </div>
