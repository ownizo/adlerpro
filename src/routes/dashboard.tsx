...
const POLICY_TYPE_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  auto:                 { bg: 'var(--status-info-bg)', text: 'var(--status-info-text)', dot: '#3B82F6' },
  health:               { bg: 'var(--status-success-bg)', text: 'var(--status-success-text)', dot: '#22C55E' },
  property:             { bg: 'var(--status-warning-bg)', text: 'var(--status-warning-text)', dot: '#F97316' },
  liability:            { bg: 'var(--status-info-bg)', text: 'var(--status-info-text)', dot: '#A855F7' },
  workers_comp:         { bg: 'var(--status-danger-bg)', text: 'var(--status-danger-text)', dot: '#F43F5E' },
  cyber:                { bg: 'var(--status-info-bg)', text: 'var(--status-info-text)', dot: 'var(--status-info-text)' },
  directors_officers:   { bg: 'var(--status-neutral-bg)', text: 'var(--status-neutral-text)', dot: 'var(--color-subtle)' },
  business_interruption:{ bg: 'var(--status-warning-bg)', text: 'var(--status-warning-text)', dot: '#F59E0B' },
  life:                 { bg: 'var(--status-success-bg)', text: 'var(--status-success-text)', dot: 'var(--status-success-text)' },
  other:                { bg: 'var(--color-surface)', text: 'var(--color-secondary)', dot: '#999999' },
}
  ...
  const nextAction = useMemo(() => {
    if (renewalAlerts.length > 0) {
      ...
      return {
        ...
        color: days <= 30 ? 'var(--status-danger-bg)' : 'var(--status-warning-bg)',
        borderColor: days <= 30 ? 'var(--status-danger-border)' : 'var(--status-warning-border)',
      }
    }
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto">
        ...
        {/* Page title */}
        <div className="mb-6">
          <h1 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '1.4rem', color: 'var(--color-primary)', margin: 0 }}>
            {t('dashboard.title')}
          </h1>
          <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: '0.85rem', color: 'var(--color-label)', marginTop: '0.25rem' }}>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--color-border)', borderTopColor: '#C8961A' }} />
          </div>
        ) : (
          <>
            {/* KPI Cards */}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '0.85rem', color: 'var(--color-primary)', margin: 0 }}>
                  {nextAction.title}
                </p>
                <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: '0.8rem', color: 'var(--color-body)', margin: '0.2rem 0 0' }}>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 flex flex-col gap-6">
                ...
                {renewalAlerts.length > 0 && (
                  <div style={{ background: 'var(--color-base)', border: '1px solid var(--color-border)', borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '0.9rem', color: 'var(--color-primary)', margin: 0 }}>
                        {t('dashboard.upcomingRenewals')}
                      </h2>
                      <Link to="/policies" style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.75rem', color: 'var(--color-accent)', fontWeight: 600, textDecoration: 'none' }}>
                        {t('dashboard.seeAll')}
                      </Link>
                    </div>
                    <div>
                      {renewalAlerts.map((policy) => {
                        ...
                        const urgency = days <= 14 ? '#EF4444' : days <= 30 ? '#F59E0B' : 'var(--color-accent)'
                        return (
                          <div
                            key={policy.id}
                            style={{
                          ...
                          >
                            ...
                            />
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontSize: '0.82rem', color: 'var(--color-primary)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {POLICY_TYPE_LABELS[policy.type] || policy.type} — {policy.insurer}
                              </p>
                              <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: '0.72rem', color: 'var(--color-label)', margin: 0 }}>
                            ...
                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                              ...
                              <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.65rem', color: 'var(--color-label-light)', margin: 0 }}>
                ...
                {premiumByType.length > 0 && (
                  <div style={{ background: 'var(--color-base)', border: '1px solid var(--color-border)', borderRadius: '4px', padding: '1.25rem' }}>
                    <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: '0.9rem', color: 'var(--color-primary)', margin: '0 0 1rem' }}>
                        ...
                        return (
                          <div key={type}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                              <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.75rem', color: 'var(--color-body)', fontWeight: 500 }}>
                                {POLICY_TYPE_LABELS[type as keyof typeof POLICY_TYPE_LABELS] || type}
                              </span>
                              <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.75rem', color: 'var(--color-primary)', fontWeight: 600 }}>
                                {formatCurrency(value)}
                              </span>
                            </div>
                            <div style={{ height: '6px', background: 'var(--color-elevated)', borderRadius: '3px', overflow: 'hidden' }}>
                              <div
                                style={{
                                  height: '100%',
                                  width: `${pct}%`,
                ...
                <div style={{ background: 'var(--color-base)', border: '1px solid var(--color-border)', borderRadius: '4px', overflow: 'hidden' }}>
                  ...
                  {policies.length === 0 ? (
                    <div style={{ padding: '2rem', textAlign: 'center' }}>
                      <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.85rem', color: 'var(--color-label-light)', margin: '0 0 0.75rem' }}>
                        {t('dashboard.noPolicies')}
                      </p>
                      <Link to="/policies" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontSize: '0.8rem', color: '#ffffff', background: 'var(--color-inverse)', padding: '0.5rem 1rem', borderRadius: '4px', textDecoration: 'none' }}>
              ...
              <div style={{ background: 'var(--color-base)', border: '1px solid var(--color-border)', borderRadius: '4px', overflow: 'hidden' }}>
                ...
                <div style={{ maxHeight: '420px', overflowY: 'auto' }}>
                  {alerts.length === 0 ? (
                    <div style={{ padding: '2rem', textAlign: 'center' }}>
                      <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.82rem', color: 'var(--color-label-light)', margin: 0 }}>
                    ...
                    alerts.map((alert) => (
                      <div
                        ...
                        style={{
                          padding: '0.75rem 1.25rem',
                          borderBottom: '1px solid var(--color-border)',
                          background: !alert.read ? 'var(--color-accent-muted)' : 'transparent',
                        }}
                      >
                        <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'flex-start' }}>
                          <AlertIcon type={alert.type} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontSize: '0.8rem', color: 'var(--color-primary)', margin: 0 }}>
                              {alert.title}
                            </p>
                            <p style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 300, fontSize: '0.72rem', color: 'var(--color-label)', margin: '0.2rem 0 0', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                              {alert.message}
                            </p>
                            <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.65rem', color: 'var(--color-subtle)', margin: '0.2rem 0 0' }}>
                              {formatDate(alert.createdAt)}
                            </p>
                          </div>
                          {!alert.read && (
                            <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: 'var(--color-accent)', flexShrink: 0, marginTop: '4px' }} />
                          )}
                        </div>
                      </div>
                    ))
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <div>
          <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '0.7rem', color: 'var(--color-label)', margin: 0, fontWeight: 400 }}>
            {label}
          </p>
          <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-primary)', margin: 0, lineHeight: 1.2 }}>
            {value}
          </p>
        </div>
      </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const styles: Record<string, { bg: string; color: string }> = {
    ...
    expiring:  { bg: 'var(--status-warning-bg)', color: 'var(--status-warning-text)' },
    expired:   { bg: 'var(--status-danger-bg)', color: 'var(--status-danger-text)' },
    cancelled: { bg: 'var(--status-neutral-bg)', color: 'var(--status-neutral-text)' },
  }
  ...
  const s = styles[status] || { bg: 'var(--status-neutral-bg)', color: 'var(--status-neutral-text)' }
  return (
...
function AlertIcon({ type }: { type: string }) {
  const icons: Record<string, { color: string; path: string }> = {
    ...
    document:     { color: 'var(--status-neutral-text)', path: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
