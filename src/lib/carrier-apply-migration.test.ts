...
test('policy participants migration is additive, constrained, RLS-protected, and idempotent', () => {
  ...
  assert.match(participantSrc, /ON CONFLICT \(policy_id, role, COALESCE\(individual_client_id::text, company_id\)\) DO NOTHING/)
  assert.match(participantSrc, /selected_policyholder_individual_client_id = CASE[\s\S]*participant_ind/)
  assert.match(participantSrc, /selected_policyholder_company_id = CASE[\s\S]*participant_company/)
  assert.match(participantSrc, /r\.selected_policyholder_individual_client_id := participant_ind/)
  assert.match(participantSrc, /r\.selected_policyholder_company_id := participant_company/)
  assert.match(participantSrc, /IF r\.selected_policyholder_individual_client_id IS NOT NULL THEN/)
  assert.match(participantSrc, /IF r\.selected_policyholder_company_id IS NOT NULL THEN/)
  assert.match(participantSrc, /SELECT e\.individual_client_id INTO participant_ind[\s\S]*ORDER BY e\.last_seen_at DESC NULLS LAST/)
  assert.match(participantSrc, /SELECT e\.company_id INTO participant_company[\s\S]*ORDER BY e\.last_seen_at DESC NULLS LAST/)
  assert.match(participantSrc, /external client identity .*already linked to a different CRM customer/)
  assert.match(participantSrc, /owner_ind := p\.individual_client_id;\s*owner_company := NULLIF\(BTRIM\(p\.company_id\), ''\)/)
  assert.match(participantSrc, /selected_individual_client_id = COALESCE\(owner_ind, selected_individual_client_id\)/)
  assert.match(participantSrc, /participant_ind[\s\S]*participant_company|participant_company[\s\S]*participant_ind/)
  assert.doesNotMatch(participantSrc, /SECURITY DEFINER/)
})

test('create-mode participant persistence and retry semantics are explicit for Bella/Ilya and company re-entry', () => {
  const participantSrc = readFileSync(join(migrationsDir, '20260831_crm3_policy_participants.sql'), 'utf8')

  assert.match(participantSrc, /WHEN 'create_individual' THEN[\s\S]*r\.selected_policyholder_individual_client_id := participant_ind/)
  assert.match(participantSrc, /WHEN 'create_company' THEN[\s\S]*r\.selected_policyholder_company_id := participant_company/)
  assert.match(participantSrc, /participant_ind IS NOT NULL THEN\s*IF EXISTS \(\s*SELECT 1 FROM public\.external_client_identities e\s*WHERE e\.provider = r\.provider/)
  assert.match(participantSrc, /participant_company IS NOT NULL THEN\s*IF EXISTS \(\s*SELECT 1 FROM public\.external_client_identities e\s*WHERE e\.provider = r\.provider/)
  assert.match(participantSrc, /already linked to a different CRM customer.*r\.provider, normalized_external_client_id/)
  assert.match(participantSrc, /INSERT INTO public\.policy_participants \(policy_id, individual_client_id, company_id, role, provider, external_client_id, source\)[\s\S]*ON CONFLICT \(policy_id, role, COALESCE\(individual_client_id::text, company_id\)\) DO NOTHING/)
  assert.match(participantSrc, /selected_policyholder_individual_client_id = CASE\s*WHEN r\.selected_policyholder_mode IN \('existing_individual', 'create_individual'\) THEN participant_ind/)
  assert.match(participantSrc, /selected_policyholder_company_id = CASE\s*WHEN r\.selected_policyholder_mode IN \('existing_company', 'create_company'\) THEN participant_company/)
  assert.match(participantSrc, /selected_individual_client_id = COALESCE\(owner_ind, selected_individual_client_id\)/)
  assert.match(participantSrc, /selected_company_id = COALESCE\(owner_company, selected_company_id\)/)
})
